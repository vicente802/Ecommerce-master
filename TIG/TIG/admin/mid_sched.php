<?php 
      ...
      $midSem = mysqli_fetch_array($firstSql); 
?>
                <div id="1" class="tabdiv">
                      <ul class="nav nav-tabs active" id="tabs" >
                          <?php
                            ...
                            $subjsql=mysqli_query($conn,"select * FROM major 
                              ...
                              WHERE major.program_id='1' AND major.sem_id='".$midSem['sem_id']."' ORDER BY major_id ASC");
                      ...
                      <?php
                        ...
                        while($secRow=mysqli_fetch_array($subjsql2)) { ?>
                           
                        <div id="tab-<?= $counter2++; ?>" class=" tab-pane"><br>

                              <form method="POST" action="mid_year_print.php<?php echo '?id='.$secRow['major_id']; ?>">
                                ...
                                <tbody>
                                  <?php
                                    $query=mysqli_query($conn,"select * from subject
                                      ...
                                      left join semester on semester.sem_id=subject.subjects_id
                                      where subject.program_id = '".$secRow['major_id']."' AND subject.sem_id='".$midSem['sem_id']."' ");
...
<!------------------------------------------------------------------------- MAED ------------------------------------------------------------->
                ...
                <div id="2" class="tabdiv" style="display: none;">
                      <ul class="nav nav-tabs"  >
                          <?php
                            ...
                            $subjsql2=mysqli_query($conn,"select * FROM major 
                              ...
                              LEFT JOIN courses on courses.program_id=subject.subjects_id 
                              LEFT JOIN semester on semester.sem_id=subject.subjects_id
                              WHERE major.program_id='2' AND major.sem_id='".$midSem['sem_id']."' ORDER BY major_id ASC");
                      ...
                      <?php
                        ...
                        <div id="tabs-<?= $counters2++; ?>" class="tab-pane con"><br>
                            <div>

                              <form method="POST" action="print_mid_maed.php<?php echo '?id='.$srow2['major_id']; ?>">
                                <button type="submit" class="btn btn-warning col-md-2" style="margin-bottom: 7px; float: right;" name="generate_pdf"><i class="fas fa-print"></i> Print Schedule</button>
                              </form>
                                ...
                                <tbody>
                                  <?php
                                    $query2=mysqli_query($conn,"select * from subject
                                      ...
                                      left join school_year on school_year.school_yr_id=schedule.school_yr_id 
                                      LEFT JOIN semester on semester.sem_id=subject.subjects_id 
                                      where subject.program_id = '".$srow2['major_id']."' AND subject.sem_id='".$midSem['sem_id']."'  ");
                                      ...
                                      <tr>
                                        ...
                                        <td>
                                          <?php  
                                            ...
                                            <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#add_sched2<?php echo $subjrow2['subjects_id']; ?>"><i class="fas fa-clock"></i> Add Schedule</button>
                                            <?php include('add_sched.php'); ?>
                                          <?php }else{ ?>
